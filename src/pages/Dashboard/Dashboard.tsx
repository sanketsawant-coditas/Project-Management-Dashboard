...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        if (isAdmin) {
          // Admin/Super Admin: fetch statistics
    ...
    return (
      <div className={styles.container}>
        ...
        <div className={styles.card}>
          ...
          <p><strong>Status:</strong> <Badge variant={user?.isActive ? 'success' : 'danger'}>{user?.isActive ? 'Active' : 'Inactive'}</Badge></p>
        </div>
        <div className={styles.card}>
          ...
          {assignedProjects.length === 0 ? (
            ...
            <ul>
              {assignedProjects.map((p: any) => (
                <li key={p.id}>{p.name} – {p.status} – {p.progress}%</li>
              ))}
            </ul>
          )}
        </div>
